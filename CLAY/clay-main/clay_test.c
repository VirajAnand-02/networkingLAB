...
#include "clay.h" // Using standard include path
// If clay.h is in a different directory, use the full path:
// #include "path/to/clay.h"
    ...
    return (Clay_Dimensions){
        .width = text.length *
                 config->fontSize,  // <- this will only work for monospace
                                    // fonts, see the renderers/ directory for
...
int main() {
    ...
    while (renderLoop()) {  // Will be different for each renderer / environment
        ...
        Clay_UpdateScrollContainers(
            true, (Clay_Vector2){mouseWheelX, mouseWheelY}, deltaTime);

        // All clay layouts are declared between Clay_BeginLayout and
        // Clay_EndLayout
        Clay_BeginLayout();
        ...
        for (int i = 0; i < renderCommands.length; i++) {
            Clay_RenderCommand *renderCommand =
                &renderCommands.internalArray[i];
