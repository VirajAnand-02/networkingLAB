...
// Let's generate a different simple pair: p=13, q=23 => n=299, phi=264. e=7, d=151
...
unsigned __stdcall ClientReceiveThreadFunc(void* args) {
    ...
    while (g_connection_active) {
        ...
        if (recv_len > 0) {
            ...
            std::string decrypted_message = decrypt(ciphertext, client_private_key);
            ...
            int error_code = WSAGetLastError();
             if (g_connection_active) { // Avoid error msg if we initiated disconnect
...
int main() {
    ...
    if (fgets(server_ip_str, sizeof(server_ip_str), stdin) != NULL) {
        ...
        if (server_ip_str[0] == '\0') { strcpy(server_ip_str, DEFAULT_IP); printf("Using default IP: %s\n", server_ip_str); }
    } else { /* ... error handling ... */ return 1; }

    printf("Enter server port (e.g., %d): ", DEFAULT_PORT);
    if (scanf("%d", &server_port_num) != 1) { server_port_num = DEFAULT_PORT; fprintf(stderr, "Invalid port. Using default %d.\n", server_port_num); }
    ...
    if ((g_sock = socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) { /* ... error handling ... */ WSACleanup(); return 1; }
    ...
    if (server_addr.sin_addr.s_addr == INADDR_NONE) { /* ... error handling ... */ closesocket(g_sock); WSACleanup(); return 1; }
    ...
    try {
        ...
        int key_recv_len = recv(g_sock, comm_buffer, sizeof(comm_buffer) - 1, 0);
        if (key_recv_len <= 0) {
        ...
        snprintf(comm_buffer, sizeof(comm_buffer), "%lld %lld", client_public_key[0], client_public_key[1]);
    ...
    RECEIVER_THREAD_ARGS* args = (RECEIVER_THREAD_ARGS*)malloc(sizeof(RECEIVER_THREAD_ARGS));
    if (!args || !hRecvEvent) { /* ... handle allocation errors ... */
         if (hRecvEvent) CloseHandle(hRecvEvent);
         if (args) free(args);
         closesocket(g_sock); g_connection_active = false; WSACleanup(); return 1;
    }
    args->hEvent = hRecvEvent;

    HANDLE hRecvThread = (HANDLE)_beginthreadex(NULL, 0, &ClientReceiveThreadFunc, args, 0, NULL);
