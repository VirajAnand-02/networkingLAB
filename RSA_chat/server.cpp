...
SOCKET g_client_socket = INVALID_SOCKET; // Make socket accessible globally (use carefully!)
...
unsigned __stdcall ReceiveThreadFunc(void* args) {
    ...
    while (g_client_connected) {
        ...
        if (recv_len > 0) {
            ...
            std::string received_serialized(recv_buffer); // Convert to C++ string
            ...
            if (ciphertext.empty() && !received_serialized.empty()) {
                 printf("\n[System] Received potentially invalid/empty ciphertext.\n> ");
                 continue; // Skip processing if deserialization likely failed
            }
            std::string decrypted_message = decrypt(ciphertext, server_private_key);
            ...
            if (g_client_connected) { // Avoid error msg if we initiated disconnect
    ...
    if(thread_args && thread_args->hEvent) SetEvent(thread_args->hEvent); // Signal completion
...
int main() {
    ...
    if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0) { /* ... error handling ... */ return 1; }
    printf("Initialised.\n");

    server_socket = socket(AF_INET, SOCK_STREAM, 0);
    if (server_socket == INVALID_SOCKET) { /* ... error handling ... */ WSACleanup(); return 1; }
    ...
    if (bind(server_socket, (struct sockaddr*)&server_addr, sizeof(server_addr)) == SOCKET_ERROR) { /* ... error handling ... */ closesocket(server_socket); WSACleanup(); return 1; }
    printf("Bind done.\n");

    if (listen(server_socket, 1) == SOCKET_ERROR) { /* ... error handling ... */ closesocket(server_socket); WSACleanup(); return 1; }
    ...
    while (1) {
        ...
        g_client_socket = accept(server_socket, (struct sockaddr*)&client_addr, &client_addr_len);

        if (g_client_socket == INVALID_SOCKET) { /* ... error handling ... */ continue; }
        ...
        try {
            ...
            snprintf(comm_buffer, sizeof(comm_buffer), "%lld %lld", server_public_key[0], server_public_key[1]);
            printf("[System] Sending server key: %s\n", comm_buffer);
            if (send(g_client_socket, comm_buffer, strlen(comm_buffer), 0) <= 0) {
                throw std::runtime_error("Failed to send server key");
            }

            // 2. Receive Client Public Key {e, n}
            memset(comm_buffer, 0, sizeof(comm_buffer));
            int key_recv_len = recv(g_client_socket, comm_buffer, sizeof(comm_buffer) - 1, 0);
            if (key_recv_len <= 0) {
        ...
        HANDLE hRecvEvent = CreateEvent(NULL, TRUE, FALSE, NULL); // Manual reset, initially non-signaled
        THREAD_ARGS* args = (THREAD_ARGS*)malloc(sizeof(THREAD_ARGS));
        if (!args || !hRecvEvent) { /* ... handle allocation errors ... */
             if (hRecvEvent) CloseHandle(hRecvEvent);
             if (args) free(args);
             closesocket(g_client_socket); g_client_connected = false; continue;
        }
        args->hEvent = hRecvEvent;

        HANDLE hRecvThread = (HANDLE)_beginthreadex(NULL, 0, &ReceiveThreadFunc, args, 0, NULL);
         if (hRecvThread == NULL) { /* ... handle thread creation error ... */
             CloseHandle(hRecvEvent); free(args);
             closesocket(g_client_socket); g_client_connected = false; continue;
         }

        // --- Main Thread Handles Sending ---
        printf("> ");
        std::string line_buffer; // Use C++ string for input reading flexibility if needed
        while (g_client_connected) {
            ...
            if (fgets(comm_buffer, sizeof(comm_buffer), stdin) != NULL) {
                 ...
                 std::string plaintext_message(comm_buffer); // Convert to C++ string
                 ...
                 std::vector<long long> ciphertext = encrypt(plaintext_message, client_public_key);
                 std::string serialized_ciphertext = serialize_ciphertext(ciphertext);

                 // Send encrypted message
                 if (send(g_client_socket, serialized_ciphertext.c_str(), serialized_ciphertext.length(), 0) <= 0) {
                      if (!g_client_connected) { /* Client disconnected concurrently */ }
                      else { /* Handle send error */ fprintf(stderr, "[System] send failed. Error Code: %d\n", WSAGetLastError()); g_client_connected = false; }
                      break;
                 }
                 printf("> "); // Re-prompt
            } else {
        ...
        // Optionally send an unencrypted disconnect notification? Risky if receiver expects encrypted.
        // Better: rely on socket closure.
        ...
        if (hRecvThread) {
             ...
             CloseHandle(hRecvThread); // Close thread handle regardless of wait result
        }
